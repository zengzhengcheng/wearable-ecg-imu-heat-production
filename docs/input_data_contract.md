...
The locked model is explicitly chamber-informed. Alongside ECG, IMU and body weight, it receives calorimetry-related time/phase and source-window fields, respiration-chamber identity, chamber temperature and relative humidity. The actual per-fold input lists are deposited under `reference_results/` and can be checked without fitting by running `scripts/05_audit_locked_model_inputs.py`.
